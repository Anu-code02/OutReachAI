...
Overview
-
This project analyzes a LinkedIn influencer's historical posts to extract topics and writing style, then uses those examples with a large language model (LLM) to generate new LinkedIn posts that match the influencer's tone, language, and preferred length. The app exposes a Streamlit UI for selecting topic, length, language, tone, formatting style, and an optional custom prompt, while maintaining session history for easy copying.
...
- **Tone and Formatting Control**: Select emotional resonance (e.g., Professional, Humorous) and visual structure (e.g., Bulleted lists).
- **Session History & Copy**: Automatically saves generated posts in collapsible expanders with 1-click copy-to-clipboard functionality.
- **Trending Hashtags**: Automatically appends contextually relevant trending hashtags to generated posts.
...
-
- `main.py` — Streamlit app: UI components for Topic, Length, Language, Tone, Formatting, Custom prompt, Generate button, and Session History.
...
flowchart LR
  subgraph UI
    U[User: upload/select posts, set topic/length/language/tone/format, custom prompt]
    S[Streamlit App]
  end

  subgraph Backend
    P[Preprocessor - preprocess]
    FSP[FewShotPosts - few_shot]
    PG[Post Generator - post_generator]
    LLM[LLM Service - GROQ]
    DS[Data - processed_posts]
  end
...
-
1. User selects Topic, Length, Language, Tone, and Formatting Style, and optionally enters a Custom Prompt in the UI.
2. The UI asks `FewShotPosts` for examples for the selected topic/length/language.
3. `PostGenerator` builds a prompt (or uses custom prompt), appends up to two examples, applies constraints/hashtags, and sanitizes the final text.
4. The app invokes the GROQ LLM via `llm_helper.py` and receives generated content.
5. Generated content is saved to session history and shown to the user in the Streamlit UI with easy copy functionality.

Files and responsibilities (detailed)
-
- `main.py` — Presents five dropdowns (Topic, Length, Language, Tone, Formatting) and a `Custom prompt` textarea. Calls `generate_post()` on button press. Renders generated posts into an interactive history list using `st.code()` for auto copy-to-clipboard functionality.
...
- `post_generator.py` — `get_prompt()` constructs the default prompt adding tone and formatting rules plus trending hashtags instruction. `generate_post()` accepts `custom_prompt` , `tone`, and `format_style` optional arguments, appends few-shot examples when available, sanitizes text (removes Unicode surrogates), and calls `llm.invoke()`.
