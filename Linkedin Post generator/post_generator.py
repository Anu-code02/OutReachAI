...
def generate_post(length, language, tag, tone="Professional", format_style="Standard paragraphs", custom_prompt: str | None = None):
    ...
    if custom_prompt and custom_prompt.strip():
        ...
        if len(examples) > 0:
            ...
            prompt += "\n\nPlease also include trending hashtags related to the topic at the end of the post."
        ...
        prompt = get_prompt(length, language, tag, tone, format_style)
...
def get_prompt(length, language, tag, tone, format_style):
    ...
    3) Language: {language}
    4) Tone: {tone}
    5) Formatting Style: {format_style}
...
if __name__ == "__main__":
    print(generate_post("Medium", "English", "Mental Health", "Professional", "Standard paragraphs"))